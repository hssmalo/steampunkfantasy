...
- Fix miscellaneous field errors: `psycic` → `psychic`, `equipment_limit` → `equipment_limit`, `race = "Ork"` → `race = "ork"`, missing `shaken` and `equipment` fields, invalid `ap` string values  # typos: ignore
