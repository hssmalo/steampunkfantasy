...
The system SHALL provide a function to retrieve race metadata for a given army name.

#### Scenario: Valid army name returns RaceConfig
- **WHEN** `get_race(army_name)` is called with a valid army name
- **THEN** it returns a `RaceConfig` object with the army's metadata (e.g., display name)
