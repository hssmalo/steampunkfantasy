...
Model: a single purchase equips every Model that can carry it with one copy and
is charged a single time, however few Models that turns out to be — a Fixture
requiring `type:Tinkerer` reaches only the Unit's Tinkerer. A Fixture may be
bought more than once, and N purchases cost N times its Cost and apply their
effects N times. Every other Upgrade is priced per Model: each Model carrying
one adds its Cost again.
