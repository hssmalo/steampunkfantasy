...
Race: the arithmetic only reads `requires` and `equipment_limit`, so a real Race
...
def test_default_survives_an_upgrade_in_a_different_holder() -> None:
    ...
    assert holders.retained_defaults(config, [gun], [mortar]) == [gun]
...
def test_default_is_evicted_by_an_upgrade_in_the_same_holder() -> None:
    ...
    assert holders.retained_defaults(config, [rifle], [crossbow]) == []
...
def test_every_default_is_kept_when_nothing_is_upgraded() -> None:
    ...
    assert holders.retained_defaults(config, [rifle, grenade], []) == [rifle, grenade]
...
def test_a_default_claiming_no_holder_is_never_evicted() -> None:
    ...
    assert holders.retained_defaults(config, [main_gun], [crossbow]) == [main_gun]
...
def test_eviction_follows_declaration_order() -> None:
    ...
    assert holders.retained_defaults(config, [first, second], [pistol]) == [first]
...
def test_an_unlimited_holder_evicts_nothing() -> None:
    ...
    assert holders.retained_defaults(config, [banner], [horn]) == [banner]
...
def test_defaults_that_over_claim_their_own_limits_are_dropped() -> None:
    ...
    assert holders.retained_defaults(config, [sword], []) == []
...
def test_upgrades_are_never_evicted_even_when_they_over_claim() -> None:
    ...
    assert holders.retained_defaults(config, [knife], [first, second]) == []
