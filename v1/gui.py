...
app.layout = html.Div(
    [
        ...
        html.Div(id="funds", children="Left FUNDS = "),
        dcc.Dropdown(options=[], id="gametype", placeholder="Choose a game type"),
        dcc.Dropdown(options=[], value="", id="team", placeholder="Select a team"),
        dcc.Dropdown(options=[], value="", id="unitname", placeholder="Select a unit"),
        dcc.Dropdown(
            options=[],
            value="N.A",
            id="upgradename",
            placeholder="Select a upgrade if any",
        ),
        dcc.Dropdown(
            options=[
                {"label": "New Unit", "value": "new"},
                {"label": "Modify unit", "value": "modify"},
            ],
            value="new",
            id="newORmodify",
        ),
        dcc.Input(
            id="personalname",
            type="text",
            placeholder="Enter personalised unit name. Must be a unique name for each unit",
        ),
        html.Button("Save", id="submit", n_clicks=0),
        html.Div(id="output-container", children="Choose A team."),
    ]
)
...
def update_output(n_clicks, newORmodify, unitname, upgradename, personalname):
    ...
    if unitname:
        ...
        if FUNDS is not None:
            ...
            txt = "Unit Name: {},Upgrade Name {},Personal Name {}".format(
                unitname, upgradename, personalname
            )
        ...
        options_gametype = [
            {"label": a + " " + b, "value": b} for a, b in build.ARMIES.items()
        ]
