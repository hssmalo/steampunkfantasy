...
class Team:
    ...
    def write_pdf(self):
        ...
        for unit_name in sorted(self.units, key=self.unit_sort):
            ...
            for weapon_name in unit.weapons_input:
                if weapon_name:
                    ...
                    latex_weapons = latex_weapons + self.weapon_template.format(
                        **weapon.__dict__
                    )
            ...
            if unit.isReplacement == "yes":
                latex_unit = latex_unit + self.model_replacement_template.format(
                    **combined_dict
                )
            else:
                latex_unit = latex_unit + self.unit_base_template.format(
                    **combined_dict
                )
            # print(latex_unit)
        ...
        for weapon_name in sorted(self.weapons.keys()):
            if weapon_name:
                ...
                if weapon.cost:
                    ...
                    latex_equipment_upgrade = latex_equipment_upgrade + template.format(
                        **weapon.__dict__
                    )
        ...
        for ability_name in self.abilities.keys():
            ...
            latex_abilities = latex_abilities + self.abilities_template.format(
                **combined
            )
        ...
        for weapon in self.weapons_input:
            if type(weapon) != Weapon:
                if weapon == "":
                    ...
                    self.orders = [
                        i for i in self.orders if i not in tmp_weapon.orders_lost
                    ]
        ...
        for key in sorted(self.__dict__.keys()):
            ...
            if self.__dict__[key] == {}:
                ...
                while more:
                    ...
                    more2 = True
                    while more2:
                        self.__dict__[key][name].append(
                            input("add an input for " + name + ":")
                        )
...
class Costimized_unit(Unit):
    ...
    def add_replacement(self, replacement):
        ...
        try:
            costimized_unit.replacements[replacement.name] = (
